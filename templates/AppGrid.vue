...
const openWindow = (windowId) => {
  const win = windowsStore.getWindowById(windowId)
  if (win && win.externalLink) {
    window.open(win.externalLink, '_blank')
    return
  }
  const payload = {
    windowState: "open",
...
<template>
  <nav class="grid-container" :style="{ height: gridHeight }">
    <li v-for="window in windowsStore.windows" :key="window.key">
      <button class="icon" v-if="window.showInAppGrid != false" @touchstart="openWindow(window.windowId)"
